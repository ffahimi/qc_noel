        ...
        for symbol, symbolData in self.symbol_data_by_symbol.items():
            ...
            if symbolData.wt.Value > 70:
                if symbolData.alma.Value < symbolData.gma.Value:
                    insights.append(Insight.Price(symbolData.symbol, timedelta(minutes=2),InsightDirection.Down)) 
            
            if symbolData.wt.Value < -70:
                if symbolData.alma.Value >= symbolData.gma.Value:
                    insights.append(Insight.Price(symbolData.symbol, timedelta(minutes=2), InsightDirection.Up))
