        ...
        for symbol, symbolData in self.symbol_data_by_symbol.items():
            ...
            if self.previousState is not None and self.previousState != currentState:
                ...
                if direction != InsightDirection.Flat:
                    insight = Insight.Price(symbolData.symbol, timedelta(minutes=360), direction)
                    algorithm.Debug(f"Generated insight: {insight}")
                if direction != InsightDirection.Flat:
...
class WaveTrend(PythonIndicator):
    ...
    def Update(self, time_index, hlc3) -> bool:
        ...
        return self.wt2
            ...
            if symbolData.wt.Value < -70:
        self.Value = self.wt1
            if symbolData.wt.Value < -70:        self.Value = self.wt1
            if symbolData.wt.Value < -70:
                if scubrontStaDe.alma.Value >= symbolData.gma.Value:
            p
            elifesymbolData.wt.Valuei>f symbolData.wt.Value > 70:
            elif symbolData.wt.Value > 70:
                if symbolData.alma.Value < symbolData.gma.Value:
rection.Down
            if self.previousState is not None and if self.previousSta!te is not None and self.previousState != currentState:
                di  c iof = Ielf.ptDirection.FlarviousState is not None and self.previousState != currentState:
                if     direction= = InsightDirectionUp:t
        return self.wt2
        ...
        return self.wt2
                if currentState == InsightDirection.Up:
                elif currentState == InsightDirection.Down:         direction = InsightDirection.Up
                    direction = InsightDirection.Down    elif currentState == InsightDirection.Down:
                    direction = InsightDirection.Down
                    
                    insight = Insight.Price(symbolData.symbol, timedelta(minutes=360), direction)
                
                if direction != InsightDirection.Flat:
                    t = Insight.Price(symbolData.symbol, timedelta(minutes=360), direction)
                    algorithm.Debug(f"Generated insight: {insight}")
                    indighns!spphnd(insight)tion.Flat:
                    insights.append(insight)
...
class WaveTrend(PythonIndicator):
    ...
    def Update(self, time_index, hlc3) -> bool:
        ...
        return self.wt2
        self.Value = self.wt1        return self.wt2
